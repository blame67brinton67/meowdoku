...
function renderRoomPanel(room, me) {
  ...
  const leaderboard = state.boardView === 'points'
    ? room.stats?.length
      ? `<div class="room-leaderboard"><p class="eyebrow">LEADERBOARD</p><h3>房間積分榜</h3>${boardTabs}<ol>${room.stats.map(row => `<li><strong>${escapeHtml(row.name)}${streak(row)}</strong><span>${row.points} 分 · 完成 ${row.completed} / ${row.played} 局${row.averageMs != null ? ` · 平均 ${(row.averageMs / 1000).toFixed(1)}s` : ''}${row.bestStreak >= 2 ? ` · 最長連霸 ${row.bestStreak}` : ''}</span></li>`).join('')}</ol><small class="points-rule">完成得 N − 名次 + 1 分（N 為該局玩家數），未完成 0 分。</small></div>`
...
function joinRoom({ code, spectator }, password) {
  socket.emit('join-room', { code, name: playerName(), playerId: state.visitorId, spectator, password }, result => {
...
function bindRoomButtons() {
  ...
  document.querySelector('#restart-room')?.addEventListener('click', event => {
    ...
    socket.emit('restart-room', { code: state.room.code, playerId: state.visitorId }, result => { if (result?.error) alert(result.error); renderGame(); });
  });
