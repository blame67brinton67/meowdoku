...
function renderGame(message = '') {
  ...
  view.innerHTML = `<section class="game-layout"><div class="game-main"><div class="game-top"><button class="back-button" id="quit">← ${state.mode === 'single' ? '關卡列表' : '離開房間'}</button><div>${state.mode === 'single' ? `<p class="eyebrow">SOLO • ${puzzle.size} × ${puzzle.size}</p><h1>${escapeHtml(puzzle.name)}</h1>${ratingLine(puzzle.rating)}` : `<p class="eyebrow">ROOM ${room.code}</p><h1>${escapeHtml(room.name)}</h1>`}</div></div><div class="game-status">${state.mode === 'single' ? `<span>找出 <b>${state.cats.size} / ${puzzle.size}</b> 隻貓咪</span>` : gameStatus(room, me)}<span id="game-message">${message}</span></div>${boardArea}${footer}${nextAction}</div>${state.mode === 'multi' ? renderRoomPanel(room, me) : '<aside class="rule-card"><p class="eyebrow">RULES</p><h2>貓咪守則</h2><ul><li>每種顏色恰有一隻貓</li><li>每行、每列恰有一隻貓</li><li>貓咪之間不能相鄰</li><li>點錯一格，挑戰失敗</li></ul></aside>'}</section>`;
...
function patchGame(puzzle, room, me, isViewing, message) {
  ...
  if (panel && panel.outerHTML !== html) { panel.outerHTML = html; bindRoomButtons(); }
}
...
function renderRoomPanel(room, me) {
  ...
  const sprintSeconds = Number(room.sprintSeconds);
  const sprintChoices = [30, 60, 90, 120];
  const sprintOptions = sprintChoices.includes(sprintSeconds) ? sprintChoices : [sprintSeconds, ...sprintChoices];
  const sprintSetting = room.status === 'lobby'
    ? isHost
      ? `<label class="sprint-setting">最後衝刺時間<select id="sprint-seconds">${sprintOptions.map(seconds => `<option value="${seconds}" ${seconds === sprintSeconds ? 'selected' : ''}>${seconds} 秒</option>`).join('')}</select></label>`
...
function bindRoomButtons() {
  ...
  document.querySelector('#sprint-seconds')?.addEventListener('change', event => socket.emit('set-sprint-seconds', { code: state.room.code, playerId: state.visitorId, seconds: Number(event.target.value) }, result => result?.error && alert(result.error)));
}
